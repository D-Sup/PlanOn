...
const Toast = ({ isOpen, closeModal, props }: ToastProps) => {
  ...
  return (
    <div className="max-w-[calc(100%-60px)] z-50 fixed top-0 left-1/2 bg-input px-5 py-2 flex-center gap-[10px] font-lg text-center text-white text-md rounded-lg backdrop-blur-sm" style={{ backgroundColor: "rgba(46,46,46,0.8)", opacity: isOpen ? 1 : 0, transition: isOpen ? ".3s ease-out" : ".3s ease-in", transform: isOpen ? "translate(-50%, 0.5rem)" : "translate(-50%, -80%)" }}>
      ...
      {props.type === "message" &&
        <>
          ...
          <span className="text-nowrap text-lg leading-none">{props.title}</span>
        </>
      }
